...
def process_images_batch(image_paths, model, authors, encoded_cache=None):
    ...
    for path in image_paths:
        ...
        if encoded_cache is not None:
            base64_thumbnail = encoded_cache.get(path)
        if base64_thumbnail is None:
            base64_thumbnail = get_thumbnail(path)
            if base64_thumbnail and encoded_cache is not None:
                encoded_cache[path] = base64_thumbnail
        if base64_thumbnail is None:
...
class ImageTaggerApp:
    def __init__(self, master):
        ...
        self.thumbnail_size = (120, 120)  # Larger thumbnails
    ...
    def create_widgets(self):
        ...
        thumb_width = self.thumbnail_size[0] + 24
        self.tree.column("#0", width=thumb_width, stretch=tk.NO, minwidth=thumb_width)
    ...
    def request_thumbnail(self, image_path, item_id):
        if image_path in self.thumbnail_cache:
            if self.tree.exists(item_id):
                self.tree.item(item_id, image=self.thumbnail_cache[image_path])
            return

        waiters = self._thumbnail_waiters.setdefault(image_path, set())
        waiters.add(item_id)

        if image_path in self._thumbnail_loading:
            return

        self._thumbnail_loading.add(image_path)
    ...
    def _create_thumbnail_data(self, image_path):
        try:
            with Image.open(image_path) as img:
                ...
                background = Image.new('RGBA', self.thumbnail_size, (255, 255, 255, 255))
                offset_x = max(0, (target_width - img.width) // 2)
                offset_y = max(0, (target_height - img.height) // 2)
    ...
    def _apply_thumbnail(self, image_path, data):
        self._thumbnail_loading.discard(image_path)
        waiters = self._thumbnail_waiters.pop(image_path, set())
    ...
    def reset_state(self):
        ...
        self._thumbnail_loading.clear()
        self._thumbnail_waiters.clear()
        if hasattr(self, '_default_thumbnail'):
            del self._default_thumbnail
    ...
    def add_tree_item(self, filename, full_path):
        item_id = str(self.image_list[filename]["index"])
        self.tree.insert(
            "",
            "end",
            iid=item_id,
            image=self.get_default_thumbnail(),
            values=(filename, "", "", ""),
            tags=(
                'even' if self.image_list[filename]["index"] % 2 == 0 else 'odd'
            ),
        )
        self.request_thumbnail(full_path, item_id)
    ...
    def sleep_with_interrupt(self, duration, step=0.5):
        ...
        while self.is_processing:
            ...
            self.pause_event.wait(wait_time)
            if not self.is_processing:
                break
        ...
        while True:
            ...
            if len(self.request_times) == 50:
                ...
                if time_diff < 60:
                    ...
                    if not self.sleep_with_interrupt(sleep_time):
                        return {
                            p: {
                                "title": "Error Processing Image",
                                "tags": ["error"],
            ...
            try:
                ...
                # This is the specific exception for rate limits from the anthropic library
                    ...
                    if not self.sleep_with_interrupt(60):
                        return {
                            p: {
                                "title": "Error Processing Image",
                                "tags": ["error"],
